# zzz_monitors_helpers.R -- internal helpers and global bindings

#' Internal global variables for non-standard evaluation
#'
#' These symbols are used in dplyr/ggplot2 code and are declared here
#' so that \code{R CMD check} does not report "no visible binding for
#' global variable" notes.
#'
#' @name samOptiPro_internal_globals
#' @keywords internal
#' @noRd
NULL
...
#' Internal helper: Rhat barplot for a subset of nodes.
#'
#' Creates a simple barplot of Rhat by target, restricted to a given set
#' of nodes. If the required columns are missing in `diag_tbl`, the
#' function returns invisibly without error.
#'
#' @param diag_tbl data.frame produced by samOptiPro diagnostics
#'   (compute_diag_from_mcmc() / compute_diag_from_mcmc_vect()).
#'   Must contain at least a 'target' column. Ideally also 'Rhat'.
#' @param nodes character vector of node names to keep.
#' @param out_file optional path to a PNG file; if non-NULL, the plot is
#'   written to disk. Otherwise it is drawn on the current device.
#' @keywords internal
.plot_rhat_bar <- function(diag_tbl, nodes, out_file = NULL) {
  # 0) Sanity checks
  if (is.null(diag_tbl) || !is.data.frame(diag_tbl)) {
    warning(".plot_rhat_bar: 'diag_tbl' is NULL or not a data.frame; skipping.")
    return(invisible(NULL))
  }
  if (!"target" %in% names(diag_tbl)) {
    warning(".plot_rhat_bar: 'diag_tbl' has no 'target' column; skipping.")
    return(invisible(NULL))
  }
  if (!"Rhat" %in% names(diag_tbl)) {
    warning(".plot_rhat_bar: 'diag_tbl' has no 'Rhat' column; skipping.")
    return(invisible(NULL))
  }

  if (is.null(nodes) || !length(nodes)) {
    warning(".plot_rhat_bar: 'nodes' is empty; skipping.")
    return(invisible(NULL))
  }

  keep_rows <- diag_tbl$target %in% nodes
  if (!any(keep_rows)) {
    message(".plot_rhat_bar: no matching nodes in diag_tbl; nothing to plot.")
    return(invisible(NULL))
  }

  # 1) Colonnes disponibles et sûres
  desired_cols <- c("target", "Rhat", "AE_ESS_per_it", "ESS_per_sec", "Family")
  cols <- intersect(desired_cols, names(diag_tbl))
  if (!length(cols)) {
    warning(".plot_rhat_bar: none of the desired columns found in diag_tbl; skipping.")
    return(invisible(NULL))
  }

  sub <- diag_tbl[keep_rows, cols, drop = FALSE]

  # Si Rhat est entièrement NA, le plot n'a pas beaucoup de sens
  if (all(is.na(sub$Rhat))) {
    message(".plot_rhat_bar: all Rhat values are NA for selected nodes; skipping plot.")
    return(invisible(NULL))
  }

  # 2) Plot (base R simple, robuste)
  if (!is.null(out_file)) {
    dir.create(dirname(out_file), recursive = TRUE, showWarnings = FALSE)
    grDevices::png(filename = out_file, width = 800, height = 600, res = 120)
    on.exit(grDevices::dev.off(), add = TRUE)
  }

  ord <- order(sub$Rhat, decreasing = TRUE, na.last = NA)
  sub <- sub[ord, , drop = FALSE]

  bar_heights <- sub$Rhat
  names(bar_heights) <- sub$target
  ...
  graphics::barplot(
    ...
    las = 2,
    ylab = "Rhat",
    main = "Rhat by target (selected nodes)"
  )
