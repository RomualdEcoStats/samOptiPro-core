...
M3.nimble2 <- nimbleCode({
  ...
  # Process model
  for(t in 1:(n-1)){
    # OLD: N[t+1] <- N[t] * theta[t]
    # NEW: apply a non-differentiable clipping on theta[t]
    #      (introduces kinks at 0.70 and 0.95)
    N[t+1] <- N[t] * min(0.95, max(0.70, theta[t]))
  }

  # Observation model
  # OLD: Nobs[1] ~ dlnorm(meanlog = log(N[1]), sdlog = sd_obs[1])
  # NEW: introduce a discontinuous change in variance using step()
  #      (adds a jump when theta[1] > 0.85)
  Nobs[1] ~ dlnorm(meanlog = log(N[1]),
                   sdlog   = sd_obs[1] + 0.30 * step(theta[1] - 0.85))

  # OLD: for(t in 3:n){
  #         Nobs[t] ~ dlnorm(meanlog = log(N[t]), sdlog = sd_obs[t])
  #       }
  # NEW: same step-based discontinuity applied up to n-1 (theta[n] does not exist)
  for(t in 3:(n-1)){
    Nobs[t] ~ dlnorm(meanlog = log(N[t]),
                     sdlog   = sd_obs[t] + 0.30 * step(theta[t] - 0.85))
  }

  # NEW: final observation (t = n) without step(), since theta[n] is undefined
  Nobs[n] ~ dlnorm(meanlog = log(N[n]), sdlog = sd_obs[n])
})
print(M3.nimble2)
```
...
m  <- nimbleModel(code = M3.nimble2, name = "M3_2",
...
build_M <- function() list(
  ...
  cmodel   = cm,
  monitors = monitors,
  code_text = paste(deparse(M3.nimble2), collapse = "\n")
)
