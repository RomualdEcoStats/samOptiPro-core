...
```{r modelRun under NUTS,echo=FALSE,message=FALSE,warning=FALSE}
...
if  ((!file.exists(paste("gerem_nimble2_",CY,".rdata",sep=""))) ){
  ...
  res <- parLapply(cl, 1:3, function(seed){
    ...
    #1-default config
    geremconfMCMC <- configureHMC(geremModel,
                                   monitors=c("beta","logq","loga",
                                              "logRglobal","Rzone","propR","logp",
                                              "sdIA","sdIP","Rcm","sdIE","sdU", "concentration_zone"))#,onlySlice = TRUE
...
rCy=jags_mat[,paste("logRglobal[",myconstants$nbyear,"]",sep="")] #last year estimate
rCylast=jags_mat[,paste("logRglobal[",myconstants$nbyear-1,"]",sep="")] #last year estimate
